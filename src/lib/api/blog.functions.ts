...
export const listPosts = createServerFn({ method: "GET" }).handler(async (): Promise<Post[]> => {
  try {
    ...
    console.error("[listPosts] Failed to init Supabase client:", err);
    return [];
  }
});
...
export const listPublicPosts = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<{ posts: Post[]; total: number }> => {
    try {
      ...
      console.error("[listPublicPosts] Failed to init Supabase client:", err);
      return { posts: [], total: 0 };
    }
  });
...
export const getPostBySlug = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<Post | null> => {
    try {
      ...
      console.error("[getPostBySlug] Failed to init Supabase client:", err);
      return null;
    }
  });
...
export const listAllPosts = createServerFn({ method: "GET" })
  ...
  .handler(async (): Promise<Post[]> => {
    try {
      ...
      console.error("[listAllPosts] Failed to init Supabase client:", err);
      return [];
    }
  });
...
export const uploadCoverImage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: formData }): Promise<{ url: string }> => {
    try {
      ...
      if (error) {
        console.error("[uploadCoverImage] Supabase error:", error);
        throw new Error(error.message);
      }
      ...
      throw err instanceof Error ? err : new Error("Failed to upload cover image");
    }
  });
  ...
  .handler(async ({ data: formData }): Promise<Attachment> => {
    try {
      ...
      const { error } = await supabase.storage
        ...
        .upload(path, file, { contentType: file.type });
      if (error) {
        console.error("[uploadPostFile] Supabase error:", error);
        throw new Error(error.message);
      }
      ...
      console.error("[uploadPostFile] Failed:", err);
  ...
  .handler(async ({ data }): Promise<Post | null> => {
    try {
      ...
      const { data: post, error } = await supabase
        ...
        .maybeSingle();
      if (error) {
        console.error("[getPostById] Supabase error:", error);
        return null;
      }
      return post;
    } catch (err) {
      console.error("[getPostById] Failed to init Supabase client:", err);
      return null;
    }
  });
